...
function Range(lower, upper, step, precision) {
  this.lower = lower;
  this.upper = upper;
  this.step = step;
  this.precision = precision;
...
function CssSizeEditorModel() {
  ...
  this.range = new Observable(new Range(0, 1, this.step));
...
CssSizeEditorModel.prototype = {
  ...
  setRange: function(lower, upper) {
    this.range.set(new Range(lower, upper, this.step, this.precision));
  }, 
  ...
  rescale: function() {
    ...
    if (this.negativeAllowed) {
      if (value > 0) {
        this.setRange(-value, 2*value);
      }
      else if (value == 0) {
        this.setRange(-10, 10); // arbitrary ??
      }
      else { // value < 0
        this.setRange(2*value, -value);
      }
    }
    else {
      if (value == 0) {
        this.setRange(0, 10); // arbitrary
      }
      else {
        this.setRange(0, 2*value);
      }
    }
  }
};
...
function CssSizeEditorView(cssSizeEditorModel) {
  ...
  this.cssSizeEditorModel.range.onChange(function(range) {
    console.log("range.onChange, range = " + range);
