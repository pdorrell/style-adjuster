...
function ValueFormats(componentValueFormats) {
  this.componentValueFormats = componentValueFormats;
  this.matchedValueFormat = null;
  this.parsedLabels = [];
  this.values = null;
}

ValueFormats.prototype = {
  parseValue: function(value) {
    this.values = null;
    this.matchedValueFormat = null;
    this.parsedLabels = [];
    for (var i=0; i<this.componentValueFormats.length && this.values == null; i++) {
      var componentValueFormat = this.componentValueFormats[i];
      this.values = componentValueFormat.parseValue(value);
      if (this.values != null) {
        this.matchedValueFormat = componentValueFormat;
        var parserLabels = componentValueFormat.labels;
        this.parsedLabels = [];
...
ComponentsEditorModel.prototype = {
  ...
  setValueString: function(valueString) {
    this.valueFormats.parseValue(valueString);
    var parsedLabels = this.valueFormats.parsedLabels;
    ...
    for (var i=0; i<parsedLabels.length; i++) {
      ...
      editorModel.setValueString(this.valueFormats.values[label]);
    }
  }, 
  ...
  updateValue: function(label, value) {
    if (this.valueFormats) {
      this.valueFormats.updateValue(label, value);
      var newValue = this.valueFormats.buildValue();
...
var oneCssSizeValueFormat = 
  new RegexValueFormat(new RegExp(oneCssSizePattern), 
                            [null, 
                             "size1", null], {size1: ""});

var cssSize1ComponentEditorModel 
  = new ComponentsEditorModel(new ValueFormats([oneCssSizeValueFormat]), 
                              ["size1"], 
...
function cssSize4ComponentsEditorModel(allowNegative) {
  var model = new ComponentsEditorModel(new ValueFormats([fourCssSizesValueFormat]), 
                                        ["size1", "size2", "size3", "size4"], 
