...
PropertyModel.prototype = {
  ...
  getExtraEditorModel: function() {
    console.log("PropertyModel.getExtraEditorModel, value = \"" + this.value.get() + "\"");
    if (this.value.get().match(CssSizeEditorModel.prototype.cssSizeRegex)) {
      return cssSizeComponentsEditorModel;
    }
    else {
      return null;
    }
...
RegexParserAndBuilder.prototype = {
  parseValue: function(value) {
    console.log("RegexParserAndBuilder.parseValue \"" + value + "\"");
...
ValueParserAndBuilder.prototype = {
  parseValue: function(value) {
    ...
    for (var i=0; i<this.componentParsersAndBuilders.length && this.values == null; i++) {
      console.log("ValueParserAndBuilder.parseValue " + i + "th componentParsersAndBuilder");
      ...
      if (this.values != null) {
        ...
        console.log("this.parsedLabels = [" + this.parsedLabels.join(", ") + "]");
      }
    }
  }, 
...
function ComponentsEditorModel(valueParserAndBuilder, labels, editorModels) {
  ...
  this.parsedLabels.log = true;
  this.parsedLabels.description = "ComponentsEditorModel.parsedLabels";
...
ComponentsEditorModel.prototype = {
  ...
  updateValue: function(label, value) {
    console.log("updated " + label + " value \"" + value + "\"");
    if (this.valueParserAndBuilder) {
      ...
      if (newValue != null && this.updateValueHandler != null) {
        console.log(" => new value built in ComponentsEditorModel = \"" + newValue + "\"");
...
function ComponentsEditorView(componentsEditorModel) {
  ...
  componentsEditorModel.parsedLabels.nowAndOnChange(function(parsedLabels) {
    console.log("ComponentsEditorView, parsedLabels change handler");
...
ComponentsEditorView.prototype = {
  showModelsForLabels: function(parsedLabels) {
    console.log("ComponentsEditorView.showModelsForLabels [" + parsedLabels.join(", ") + "]");
    ...
    for (var i=0; i<labels.length; i++) {
      ...
      console.log("  " + label + " is " + (!active ? "NOT " : "") + "active");
      console.log("editorModel = " + editorModel);
      console.log("editorModel.view = " + editorModel.view);
      console.log("editorModel.view.dom = " + editorModel.view.dom);
...
CssSizeEditorModel.prototype = {
  ...
  setValueString: function(valueString) {
    console.log("CssSizeEditorModel.setValueString \"" + valueString + "\"");
...
var oneCssSizeParserAndBuilder = 
  new RegexParserAndBuilder(/^([-]?[0-9.]+(%|in|cm|mm|px|pt|em|ex|rem|pc))$/, 
                            ["size1", null], {size1: ""});

var cssSizeValueParserAndBuilder = new ValueParserAndBuilder([oneCssSizeParserAndBuilder]);

var cssSizeComponentsEditorModel = new ComponentsEditorModel(cssSizeValueParserAndBuilder, 
                                                             ["size1"], 
                                                             {size1: sizeEditor(false)})
new ComponentsEditorView(cssSizeComponentsEditorModel);

var cssSizeEditorModel = new CssSizeEditorModel();
new CssSizeEditorView(cssSizeEditorModel);
