...
function SliderControlView(sliderControlModel) {
  ...
  var $this = this;
  this.sliderControlModel.label.onUpdate(function(label) {
    $this.labelDom.text(label);
  });
  this.sliderControlModel.value.onUpdate(function(value) {
    $this.valueDom.text(value);
  });
  this.sliderControlModel.range.onUpdate(function(range) {
    $this.rangeDom.text("(range from " + range.lower + range.unit + " to " + range.upper + range.unit + 
                        " in steps of " + range.step + range.unit + ")");
  });
  this.editorModel.nowAndOnUpdate(function(editorModel) {
...
function CssSizeEditorModel(description, negativeAllowed) {
...
function sizeEditor(description, negativeAllowed) {
  var model = new CssSizeEditorModel(description, negativeAllowed);
...
function cssSizeEditorModel(allowNegative) {
  var model = new ComponentsEditorModel([oneCssSizeFormat], 
                                        ["size"], 
                                        {size: sizeEditor(null, allowNegative)})
...
function fourCssSizesEditorModel(allowNegative) {
  var model = new ComponentsEditorModel([fourCssSizesFormat], 
                                        ["top", "right", "bottom", "left"], 
                                        {top: sizeEditor("Top", allowNegative), 
                                         right: sizeEditor("Right", allowNegative), 
                                         bottom: sizeEditor("Bottom", allowNegative), 
                                         left: sizeEditor("Left", allowNegative)});
  new ComponentsEditorView(model);
  return model;
}
