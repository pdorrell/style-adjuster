...
function PropertyTypes() {
  this.types = {};
  this.setType(new FourCssSizesType(true), 
               ["margin"]);
  this.setType(new FourCssSizesType(false), 
               ["padding", "border-width"]);
  this.setType(new CssSizeType(false), 
               ["font-size", "width", "min-width", "max-width"]);
  this.setType(new ColorType(), 
               ["color", "background-color", "border-color"]);
  this.setType(new CssSizeType(false), 
               this.templated("padding-position", "position", ["top", "bottom", "left", "right"]))
  this.setType(new CssSizeType(true), 
               this.templated("margin-position", "position", ["top", "bottom", "left", "right"]))
}
...
PropertyTypes.prototype = {
  ...
  templated: function(template, variable, values) {
    var results = [];
    for (var i=0; i<values.length; i++) {
      results.push(template.replace(variable, values[i]));
    }
    return results;
  }
};
