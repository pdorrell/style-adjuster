...
function SliderControlView(sliderControlModel) {
  ...
  this.valueDom = $("<div class='value'/>");
  this.rangeDom = $("<div class='range'/>");
  var valueAndRangeDom = $("<div class='value-and-range'>").appendTo(this.dom);
  valueAndRangeDom.append(this.labelDom, " ", this.valueDom, " ", this.rangeDom);
  var $this = this;
  ...
  this.sliderControlModel.range.onChange(function(range) {
    if (range == null) {
      ...
      $this.rangeDom.text("(" + range.lower + range.unit + " to " + range.upper + range.unit + 
                          ", step: " + range.step + range.unit + ")");
    }
  });
...
CssSizeEditorModel.prototype = {
  ...
  resetZoom: function() {
    ...
    console.log("this.cssSize = " + this.cssSize);
    this.resetRange();
    $(this).trigger("zoomEdited");
  },
  ...
  getDescription: function(parsedLabels) {
    ...
    if (this.label == "top") {
      return numSizes == 1 ? "All" : (numSizes == 2 ? "Top/Bottom" : "Top");
    }
  ...
  resetRange: function() {
    console.log("resetRange, this.cssSize = " + this.cssSize);
  ...
  setCssSizeFromPosition: function(position) {
    ...
    console.log("setCssSizeFromPosition, this.cssSize = " + this.cssSize);
