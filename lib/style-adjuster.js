...
PropertyModel.prototype = {
  ...
  getExtraValueEditorModel: function() {
    if (this.value.get().match(CssSizeEditorModel.prototype.cssSizeRegex)) {
      console.log("cssSizeEditorModel = " + cssSizeEditorModel);
      return cssSizeEditorModel;
    }
    else {
      return null;
    }
...
ValueEditorModel.prototype = {
  ...
  editProperty: function(propertyModel) {
    ...
    this.setExtraValueEditorModelFor(propertyModel);
  }, 
  
  setExtraValueEditorModelFor: function(propertyModel) {
    var extraValueEditorModel = propertyModel.getExtraValueEditorModel();
    if (extraValueEditorModel != null) {
      extraValueEditorModel.propertyModel = propertyModel;
      extraValueEditorModel.setValueString(propertyModel.value.get());
      extraValueEditorModel.parentEditor = this;
    }
    this.extraEditorModel.set(extraValueEditorModel);
  },
  ...
  updateFromPropertyModel: function() {
    var value = this.propertyModel.get().value.get();
    this.value.set(value);
    this.changed.set(this.propertyModel.get().changed.get());
    if (this.extraValueEditorModel != null) {
      extraValueEditorModel.setValueString(this.propertyModel.value.get());
    }
  }
...
function ValueEditorView(valueEditorModel) {
  ...
  this.valueEditorModel.extraEditorModel.nowAndOnChange(function(extraEditorModel) {
    $this.extraEditorDom.children().each(function(child) {
      console.log(" detaching extraEditorDom child " + child);
      $this.extraEditorDom.detach(child);
    });
...
Range.prototype = {
  ...
  positionForValue: function(value) {
    console.log("Range.positionForValue, value = " + value);
...
CssSizeEditorModel.prototype = {
  ...
  setCssSizeFromPosition: function(position) {
    ...
    console.log("setCssSizeFromPosition, position = " + position + ", new number = " + number);
  ...
  updateValue: function(value) {
    this.propertyModel.updateValue(value);
    this.parentEditor.saveValue(value);
  }, 
  
  rescale: function() {
    var value = this.cssSize.get().number;
    console.log("rescale, value = " + value);
    ...
    console.log("range = " + this.range.get().toString());
  }
};
...
function CssSizeEditorView(cssSizeEditorModel) {
  ...
  this.dom = $("<div/>");
  this.lowerValueDom = $("<span class='value'/>").appendTo(this.dom);
  this.sliderDom = $("<div class='scale-slider'/>").appendTo(this.dom);
  this.upperValueDom = $("<span class='value'/>").appendTo(this.dom);
  this.unitDom = $("<span class='css-unit'/>").appendTo(this.dom);
  ...
  this.cssSizeEditorModel.range.onChange(function(range) {
    console.log("Range has changed to " + range.toString());
  ...
  this.cssSizeEditorModel.cssSize.onChange(function(cssSize) {
    console.log("cssSize onChange, cssSize = " + cssSize);
    $this.unitDom.text(cssSize ? cssSize.unit : null);
  });
}

var cssSizeEditorModel = new CssSizeEditorModel();
var cssSizeEditorView = new CssSizeEditorView(cssSizeEditorModel);
