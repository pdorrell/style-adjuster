...
PropertyModel.prototype = {
  ...
  updateValue: function(value) {
    ...
    if (this.type.formatsController) {
      this.type.formatsController.valueChanged(updatedValue);
    }
...
FormatsStateModel.prototype = {
  setFormatsController: function(formatsController, initialValue) {
    ...
    if (formatsController == null) {
      ...
      formatsController.valueChanged(initialValue);
    }
  }
};
...
ColorFormatsController.prototype = {
  ...
  /** Get format, assuming it is a valid color */
  getFormatOfValue: function(value) {
    var functionMatch = value.match(/^([a-z]+)\(/);
    if (functionMatch) {
    ...
    else if (colorConverter.cssKeywords[value]) {
      return "name";
    }
    else {
      return null;
  ...
  valueChanged: function(value) {
    console.log("ColorFormatsController.valueChanged = " + inspect(value));
    var valueFormat = this.getFormatOfValue(value);
    console.log(" valueFormat = " + inspect(valueFormat));
    this.formatsStateModel.selectedFormat.set(valueFormat);
    var formatsEnabled = {};
    if (valueFormat != null) { // null means not valid (shouldn't happen ??)
      for (var i=0; i<this.formats.length; i++) {
        var format = this.formats[i];
        formatsEnabled[format] = format != "name" || format == valueFormat;
      }
    }
...
function colorEditorModel() {
  var sliderControlModel = new SliderControlModel();
  var sliderControlView = new SliderControlView(sliderControlModel);
  var model = new ComponentsEditorModel([rgbColorFormat, rgbaColorFormat, hslColorFormat, hslaColorFormat], 
