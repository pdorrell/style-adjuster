...
PropertyModel.prototype = {
  ...
  getExtraEditorModel: function() {
    ...
    if (propertyEditorModel) {
      ...
      return propertyEditorModels["positiveSize"];
    }
    else {
      return null;
    }
...
var propertyEditorModels = 
  {"margin": fourCssSizesEditorModel(true), 
   "padding": fourCssSizesEditorModel(false), 
   
   // pseudo-properties
   "positiveSize": cssSizeEditorModel(false), 
   "size": cssSizeEditorModel(true)
  };
...
CssSizeDenormaliser.prototype = {
  ...
  denormalise: function(inputValue, normalisedValue) {
    ...
    var inputMatch = inputValue.match(sizeRegex);
    if (inputMatch) {
      var inputNumber = inputMatch[1];
