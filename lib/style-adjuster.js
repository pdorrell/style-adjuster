...
Range.prototype = {
  ...
  valueAtPosition: function(position) { // position 0 => lower, position 1 => upper
    console.log("Range, valueAtPosition, position = " + position);
...
CssSizeEditorModel.prototype = {
  ...
  setValueString: function(valueString) {
    ...
    console.log("CssSizeEditorModel.setValueString " + inspect(valueString));
  ...
  setCssSizeFromPosition: function(position) {
    ...
    console.log("numberString = " + inspect(numberString));
...
function CssSizeEditorView(cssSizeEditorModel) {
  ...
  function sliderChanged(event, ui) {
    if (!$this.ignoreValueChanges) {
      ...
      console.log("sliderChanged, position = " + position);
  ...
  this.cssSizeEditorModel.unit.onChange(function(unit) {
    $this.unitDom.text(unit);
  });
}

function CssType(editorModel, denormaliser) {
...
RangesForUnit.prototype = {
  getStepSizeFor: function(absCentreValue) {
    console.log("getStepSizeFor " + absCentreValue);
    for (var i=this.sizesAndSteps.length-1; i >= 0; i--) {
      var sizeAndStep = this.sizesAndSteps[i];
      console.log(" sizeAndStep " + i + " = " + inspect(sizeAndStep));
      if (i == 0 || absCentreValue >= sizeAndStep[0]) {
        console.log(" found step " + sizeAndStep[1]);
  ...
  getRange: function(value, negativeAllowed) {
    ...
    if (value < 0) {
      return new Range(centreValue*2, -centreValue, step);
    }
    else if (negativeAllowed) {
      return new Range(-centreValue, 2*centreValue, step);
    }
    else {
      return new Range(0, 2*centreValue, step);
    }
  }
};
...
var unitRanges = {
  ...
  "mm": new RangesForUnit("mm", 10, [[0, 0.5], [50, 1]]), 
  "px": new RangesForUnit("px", 10, [[0, 0.5], [30, 1]]), 
  "pt": new RangesForUnit("pt", 10, [[0, 0.5], [30, 1]]), 
  "em": new RangesForUnit("em", 1, [[0, 0.1]]), 
  "ex": new RangesForUnit("ex", 1, [[0, 0.1]]), 
  "rem": new RangesForUnit("rem", 1, [[0, 0.1]]), 
  "pc": new RangesForUnit("pc", 10,[ [0, 1]])
};
...
function cssSizeType(allowNegative) {
  return new CssType(cssSizeEditorModel(allowNegative), new CssSizeDenormaliser("em"));
}
