...
function SliderControlView(sliderControlModel) {
  ...
  this.sliderControlModel.range.onChange(function(range) {
    if (range == null) {
      ...
      $this.rangeDom.text("(" + range.lowerString + range.unit + " to " + range.upperString + range.unit + 
                          ", step: " + range.stepString + range.unit + ")");
    }
  });
...
CssSizeEditorModel.prototype = {
  ...
  zoomIn: function() {
    console.log("CssSizeEditorModel.zoomIn ...");
    this.zoomByFactor(2.0);
    $(this).trigger("zoomEdited");
  },
  
  zoomOut: function() {
    console.log("CssSizeEditorModel.zoomOut ...");
  ...
  zoomByFactor: function(zoomFactor) {
    ...
    this.range.set(rangesForUnit.getZoomedRange(value, this.negativeAllowed, 
                                                this.range.get(), zoomFactor));
  },
  
  resetZoom: function() {
    console.log("CssSizeEditorModel.resetZoom ...");
    this.resetRange();
    $(this).trigger("zoomEdited");
  },
  ...
  getDescriptionsShortAndLong: function(parsedLabels) {
    ...
    if (this.label == "top") {
      return numSizes == 1 ? ["", "All"] : (numSizes == 2 ? ["T/B", "Top/Bottom"] : ["T", "Top"]);
    }
...
function CssSizeEditorView(cssSizeEditorModel) {
  ...
  this.shortDescriptionDom = $("<span class='short-description'/>");
  ...
  this.dom.append(this.shortDescriptionDom, this.lowerValueDom, " ", this.sliderDom, 
  ...
  this.cssSizeEditorModel.focussed.nowAndOnChange(function (focussed) {
    $this.dom.toggleClass("focussed", focussed);
  });
  
  this.cssSizeEditorModel.shortDescription.nowAndOnChange(function(shortdescription) {
  ...
  $(this.cssSizeEditorModel).on("zoomEdited", function() {
...
function sizeEditor(negativeAllowed, sliderControlModel) {
...
function cssSizeEditorModel(allowNegative) {
  ...
  var model = new ComponentsEditorModel([oneCssSizeFormat], 
                                        ["size"], 
                                        {size: sizeEditor(allowNegative, sliderControlModel)}, 
                                        [sliderControlModel])
...
function fourCssSizesEditorModel(allowNegative) {
  ...
  var model = new ComponentsEditorModel([fourCssSizesFormat], 
                                        ["top", "right", "bottom", "left"], 
                                        {top: sizeEditor(allowNegative, sliderControlModel), 
                                         right: sizeEditor(allowNegative, sliderControlModel), 
                                         bottom: sizeEditor(allowNegative, sliderControlModel), 
                                         left: sizeEditor(allowNegative, sliderControlModel)}, 
                                       [sliderControlModel]);
  var componentsEditorView = new ComponentsEditorView(model);
  return model;
}
