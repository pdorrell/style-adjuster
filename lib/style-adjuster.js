...
StyleSheetModel.prototype = {
  populateRules: function() {
    ...
    for (var i=0; i<rules.length; i++) {
      var rule = rules[i];
      if(rule.type == 1) {
        this.rulesList.push(new RuleModel(this, rule));
      }
    }
  }, 
...
function RuleModel(styleSheetModel, rule) {
  ...
  toggleExpanded: function() {
    ...
    if (this.expanded && this.properties == null) {
      ...
      for (var i=0; i<style.length; i++) {
        propertyNames.push(style[i]);
      }
      propertyNames.sort();
      for (var i=0; i<propertyNames.length; i++) {
        this.properties.push(new PropertyModel(this, style, propertyNames[i]));
      }
    }
...
function PropertyModel(ruleModel, style, name) {
  this.ruleModel = ruleModel;
  this.styleAdjusterModel = ruleModel.styleAdjusterModel;
  this.style = style;
  this.name = name;
  this.value = style.getPropertyValue(name);
  this.selected = false;
  this.changed = false;
...
PropertyModel.prototype = {
  ...
  updateValue: function(newValue) {
    this.style.setProperty(this.name, newValue, null);
    this.value = this.style.getPropertyValue(this.name);
...
function CssPropertyModel(name, value) {
  this.name = name;
  this.value = value;
};
...
function CssTextModel(cssText) {
  ...
  if (openBracePos == -1) {
    throw "No opening brace in cssText: \"" + cssText + "\"";
  }
  var closeBracePos = cssText.lastIndexOf("}");
  if (closeBracePos == -1) {
    throw "No closing brace in cssText: \"" + cssText + "\"";
  }
  ...
  for (var i=0; i<propertyTexts.length; i++) {
    ...
    if (propertyText != "") {
      ...
      if (colonPos == -1) {
        throw "No colon cssText property definition: \"" + propertyText + "\"";
      }
      var name = trim(propertyText.substring(0, colonPos));
      var value = trim(propertyText.substring(colonPos+1, propertyText.length));
      this.properties.push (new CssPropertyModel(name, value));
    }
  }
}
...
RuleView.prototype = {
  ...
  updateFromModelExpansion: function() {
    ...
    if(this.ruleModel.expanded && this.propertiesDom == null) {
      ...
      this.cssTextDom = $("<pre class='css-text'/>").appendTo(this.dom);
      var cssText = this.ruleModel.rule.cssText;
      console.log("css text = " + cssText);
      var indentedCssText = indentCssText(cssText);
      console.log(" indented = " + indentedCssText);
      this.cssTextDom.text(indentedCssText);
    }
