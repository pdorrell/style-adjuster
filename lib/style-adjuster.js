...
StyleSheetModel.prototype = {
  updatePropertyValue: function(ruleIndex, name, value, cssTextModel) {
    ...
    if(!newCssTextModel.matchesUpdate(cssTextModel, name, value)) {
    ...
    return {rule: newRule, cssTextModel: newCssTextModel};
  }, 
  ...
  setExpandedOnAllVisibleRules: function(expanded) {
    for (var i=0; i<this.rulesList.length; i++) {
      var ruleModel = this.rulesList[i];
      if ((ruleModel.filtered.get() || ruleModel.selected) && (ruleModel.expanded != expanded)) {
...
Observable.prototype = {
  ...
  onChange: function(handler) {
    $(this).on("change", function(event, value) {handler(value);});
  }, 
  nowAndOnChange: function(handler) {
    handler(this.get());
...
RuleModel.prototype = {
  filterRule: function() {
    ...
    this.filtered.set(matchesFilter && (onlyShowSelected ? this.selected : true));
  }, 
  ...
  setPropertyValue: function(name, value) {
    ...
    for (var i=0; i<this.properties.length; i++) {
      if (this.properties[i].name == name) {
        this.properties[i].value = newCssPropertyModels[i].value;
      }
    }
  }, 
...
PropertyModel.prototype = {
  ...
  updateValue: function(newValue) {
    ...
    this.changed.set(this.value != this.originalValue);
...
CssTextModel.prototype = {
  ...
  matchesUpdate: function(cssTextModel, name, value) {
    ...
    for (var i=0; i<this.properties.length; i++) {
      ...
      if (property.name != name && property.value != oldProperty.value) {
        return false;
      }
    }
    return true;
...
function RuleView(ruleModel) {
  ...
  this.ruleModel.filtered.nowAndOnChange(function(filtered) {
    $this.updateFromModelFiltered(filtered);
  });
  ...
  this.ruleModel.changed.nowAndOnChange(function(changed) {
    $this.updateFromModelChanged(changed);
  });
...
RuleView.prototype = {
  ...
  updateFromModelChanged: function(changed) {
  ...
  updateFromModelSelection: function() {
    this.selectCheckbox.prop("checked", this.ruleModel.selected);
    this.dom.toggle(this.ruleModel.filtered.get() || this.ruleModel.selected);
  }, 
  
  updateFromModelFiltered: function(filtered) {
    this.dom.toggle(filtered || this.ruleModel.selected);
...
ValueEditorModel.prototype = {
  editProperty: function(propertyModel) {
    this.propertyModel = propertyModel;
    this.textValue = this.propertyModel.value;
    this.updateFlags();
  }, 
...
TextValueEditor.prototype = {
  ...
  saveValue: function() {
    ...
    try {
      ...
      if (error instanceof InvalidPropertyValueError) {
        ...
        this.showErrorMessage("Error saving property value: " + error.message);
        throw error;
      }
    }
  }, 
  ...
  updateResetButtonState: function() {
    this.resetButton.prop("disabled", !this.propertyModel.changed.get());
  },    
