...
PropertyModel.prototype = {
  ...
  updateValueViaFormatsController: function(value, formatsController) {
    ...
    if (parsedValue == null) {
      throw new InvalidPropertyValueError(this.name, value);
    }
    var formattedParsedValue = parsedValue.toString();
    this.ruleModel.setPropertyValue(this, value);
    var updatedValue = this.value.get();
    var reformattedValue = formatsController.resetUpdatedValue(updatedValue, parsedValue);
...
ValueEditorModel.prototype = {
  ...
  editProperty: function(propertyModel) {
    ...
    this.formatsStateModel.setFormatsController(propertyModel.type.formatsController, this.value.get());
  }, 
...
ComponentsEditorModel.prototype = {
  ...
  setValueString: function(valueString) {
    ...
    for (var i=0; i<parsedLabels.length; i++) {
      ...
      editorModel.setValueString(this.valueParserAndBuilder.parsedValue[label]);
...
FormatsStateModel.prototype = {
  setFormatsController: function(formatsController, initialValue) {
    ...
    if (formatsController == null) {
      ...
      formatsController.setFormatsStateModel(this);
      formatsController.setInitialValue(initialValue);
    }
  }
};
...
ColorFormatsController.prototype = {
  ...
  setFormatsStateModel: function(formatsStateModel) {
    this.formatsStateModel = formatsStateModel;
    this.formatsStateModel.formats.set(this.formats);
  }, 
  ...
  resetUpdatedValue: function(updatedValueString, parsedInputValue) {
    ...
    if (parsedUpdatedValue == null) {
      ...
      if (parsedUpdatedValue.format != parsedInputValue.format) {
        var enabledFormats = parsedUpdatedValue.enabledFormats();
        if ($.inArray(parsedInputValue.format, enabledFormats)) {
  ...
  setInitialValue: function(valueString) {
  ...
  valueChanged: function(value) {
    console.log("ColorFormatsController.valueChanged " + value);
    this.formatsStateModel.selectedFormat.set(value.format);
    var formatsEnabled = {};
    var formats = value.enabledFormats();
    for (var i=0; i<formats.length; i++) {
      formatsEnabled[formats[i]] = true;
    }
    this.formatsStateModel.formatsEnabled.set(formatsEnabled);
    this.formatsStateModel.enabled.set(true);
  }
};
...
function FormatsView(formatsStateModel) {
  ...
  var $this = this;
  this.formatsStateModel.formats.nowAndOnChange(function (formats) {
    $this.dom.toggle(formats != null);
    if (formats != null) {
      $this.setOptions(formats);
    }
  });
  this.formatsStateModel.selectedFormat.nowAndOnChange(function(selectedFormat) {
    $this.dom.find("option").each(function() {
      this.selected = (this.value == selectedFormat);
    });
  });
  this.formatsStateModel.formatsEnabled.nowAndOnChange(function(formatsEnabled) {
    $this.dom.find("option").each(function() {
      this.disabled = !formatsEnabled[this.value];
    });
  });
  this.formatsStateModel.enabled.nowAndOnChange(function(enabled) {
...
function ColorDenormaliser() {
}

ColorDenormaliser.prototype = {
  hslRegex: /^\s*hsl\s*\(([0-9]+)\s*,\s*([0-9]+)%\s*,\s*([0-9]+)%\s*\)\s*$/, 
  
  denormalise: function(inputValue, normalisedValue) {
    var denormalised = normalisedValue;
    var hslMatch = inputValue.match(this.hslRegex);
    var rgbMatch = normalisedValue.match(rgbRegex);
    if (hslMatch && rgbMatch) {
      var hue = Math.max(0, Math.min(360, parseInt(hslMatch[1])));
      var saturation = Math.max(0, Math.min(100, parseInt(hslMatch[2])));
      var lightness = Math.max(0, Math.min(100, parseInt(hslMatch[3])));
      denormalised = "hsl(" + hue + ", " + saturation + "%, " + lightness + "%)";
    }
    return denormalised;
  }
}
...
function cssSizeEditorModel(allowNegative) {
  ...
  var model = new ComponentsEditorModel(new ValueParserAndBuilder([oneCssSizeFormat]), 
                                        ["size"], 
...
function fourCssSizesEditorModel(allowNegative) {
  var sliderControlModel = new SliderControlModel();
  var sliderControlView = new SliderControlView(sliderControlModel);
  var model = new ComponentsEditorModel(new ValueParserAndBuilder([fourCssSizesFormat]), 
                                        ["top", "right", "bottom", "left"], 
...
var rgbaRegex = /rgba\(([0-9]+),\s([0-9]+),\s([0-9]+),\s([0-9.]+)\)/;

var rgbaColorFormat = 
  new RegexValueFormat(rgbaRegex, 
                       [null, "red", "green", "blue", "alpha"], 
                       {red: "rgba(", green: ", ", blue: ", ", alpha: ", "}, 
                       ")");

var hslRegex = /hsl\(([0-9]+),\s([0-9]+%),\s([0-9]+%)\)/;

var hslColorFormat = 
  new RegexValueFormat(hslRegex, 
                       [null, "hue", "saturation", "lightness"], 
                       {hue: "hsl(", saturation: ", ", lightness: ", "}, 
                       ")")

var hslaRegex = /hsla\(([0-9]+),\s([0-9]+%),\s([0-9]+)%,\s([0-9.]+)\)/;

var hslaColorFormat = 
  new RegexValueFormat(hslaRegex, 
                       [null, "hue", "saturation", "lightness", "alpha"], 
                       {hue: "hsl(", saturation: ", ", lightness: ", ", alpha: ", "}, 
                       ")")

function colorEditorModel() {
  var sliderControlModel = new SliderControlModel();
  var sliderControlView = new SliderControlView(sliderControlModel);
  var model = new ComponentsEditorModel(new ValueParserAndBuilder([rgbColorFormat, rgbaColorFormat, 
                                                                   hslColorFormat, hslaColorFormat]), 
...
function colorType() {
  return new CssType(colorEditorModel(), 
                     new ColorDenormaliser(), 
                     new ColorFormatsController(), 
                     "CSS color");
}
